...
contract DiceGame {
    ...
    function qBlockNumber_now() public view returns(uint256){
        return block.number;
    }
    ...
    function winner_generator(uint256 _a, uint256 _b, uint256 blockNumber) private view returns (uint256, uint256) {
        ...
        uint256 c = uint256(keccak256(abi.encode(_a,_b,uint256(blockhash(blockNumber))))) % 6 + 1;
        if (c<=3 && c>=1) {
    ...
    //no active created game by that user, and the user is not participating in someone else's game
     ...
     function create_PrivateGame(bytes32 num_hash_1, address participant_address) payable public {
    ...
    function create_OpenGame(bytes32 num_hash_1) payable public {
        require(Players[msg.sender].gameCreatorAddress == 0x0000000000000000000000000000000000000000, "You already in a game");
        require(msg.value == 3 ether, "You must deposit exactly 3 ETH");
        bytes32 num_hash_2_temp = 0x00000000000000000000000000000000;
    ...
    function verifyCommitment(uint256 n) public{
    ...
    function getWinner() public {
        ...
        //checking if we are within 256 block range from the blockNumber_result, if we are too far, both player are refunded
        if (block.number - blockNumber_result >= 255) {
    ...
    //if someone doen not commit ot the game or does not verify for some amount of time (blocks) the game can be canceled by any player.
    function cancelGame() public {
        ...
        if (Games[Players[msg.sender].gameCreatorAddress].num_hash_participant == 0x00000000000000000000000000000000) {   //check if the other person did not commit
            require(block.number-Games[Players[msg.sender].gameCreatorAddress].lastModified_blockNumber >= 6, "You can not yet cancel the game");
            ...
            require(block.number-Games[Players[msg.sender].gameCreatorAddress].lastModified_blockNumber >= 30, "You can not yet cancel the game");
